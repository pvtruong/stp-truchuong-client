...
var baseInput = function(code,server_path,fields_find,title,options){
		...
		appInfo.info(code,function(error,userinfo,appinfo){
			...
			if(input.loading){
				$rootScope.nextTick(function(){
					input.loading($scope,{$controller:$controller,$http:$http,service:service,$location:$location,config:config,$rootScope:$rootScope,$window:$window,$interval:$interval,$modal:$modal});
				})
					...
					service.load(id_app,{condition:condition,filter:$scope.filter,page:page,limit:$scope.limit,queryParams:queryParams,onCondition:options.onCondition})
						.success(function(data){
							async.map(data,function(d,callback){
								$http.get(server_url + "/api/follow?q={id_object:'" + d._id + "',user_created:'" + $scope.current_user.email + "'}")
									.success(function(rs){
										if(rs && rs.length>0){
											d.followObject = rs[0];
											d.follow_yn = 1;
			...
			$scope.follow = function(r){
				if(r.follow_yn==1){
					//unfollow
					$http.delete(server_url + "/api/follow/" + r.followObject._id)
					.success(function(rs){
						r.follow_yn =0;
					}).error(function(e){
						$window.alert(e);
					...
					//follow
					followObject = {id_object:r._id,collection_name:code};
					$http.post(server_url + "/api/follow",followObject)
					.success(function(rs){
						r.followObject =rs;
						r.follow_yn =1;
					}).error(function(e){
			...
			$scope.options = function(){
				...
				var modalInstance = $modal.open({
				  templateUrl: 'modules/' + input.group + "/" + code + '/templates/options.html',
	...
	this.module.controller(code + 'ViewController',['$controller','$scope','$routeParams',code + 'Config',code,'$rootScope','$location','$window','$http','$modal','appInfo','$localStorage'
		,function($controller,$scope,$routeParams,config,service,$rootScope,$location,$window,$http,$modal,appInfo,$localStorage){
		appInfo.info(code,function(error,userinfo,appinfo){
			if(!error){
				$scope.code = code;
				$scope.location = $location.url();
				...
				if($scope.list){
					for(var i=0;i<$scope.list.length;i++){
						if($scope.list[i]._id==$routeParams.id){
							...
							$scope.data.links =[];
							if(options.onView){
								options.onView($scope,{$controller:$controller,$http:$http,service:service,$location:$location,config:config,$window:$window,$modal:$modal,$rootScope:$rootScope,$localStorage:$localStorage});
							}
							break;
						}
					}
				...
				$scope.follow = function(){
					if($scope.data.follow_yn==1){
						//unfollow
						$http.delete(server_url + "/api/follow/" + $scope.data.followObject._id)
						.success(function(rs){
						...
						//follow
						followObject = {id_object:$scope.data._id,collection_name:code};
						$http.post(server_url + "/api/follow",followObject)
						.success(function(rs){
	...
	this.module.config(['$routeProvider','$locationProvider',function($routeProvider,$locationProvider){
		$routeProvider
			...
			.when("/"+code+"/json",{
				templateUrl:"bases/templates/json.html",
