angular.module('giamtruModule',[])
	.directive('ngGiamtru',[function(){
	return {
		restrict:'E',
		scope:{
			ngData:'=',
		...
		templateUrl:'templates/vouchers/giamtru/templates/table.html',
		controller:['$scope','$modal','dmkh','dmvat','$timeout',function($scope,$modal,dmkh,dmvat,$timeout){
			...
			$scope.openDetail = function (size,template) {
				var modalInstance = $modal.open({
				  templateUrl: 'templates/vouchers/giamtru/templates/edit.html',
		...
		link:function(scope,elem,attrs,ctr){
				scope.$watch('dt_current.giam_tru_nt',function(newData){
					if(scope.status.isOpened){
						scope.dt_current.giam_tru = scope.dt_current.giam_tru_nt * scope.ngMasterData.ty_gia;
					}
				});	
			}
	}
