angular.module('lichsugiaoModule',[])
	.directive('ngLichsugiao',[function(){
	return {
		restrict:'E',
		scope:{
			ngData:'=',
		...
		templateUrl:'templates/vouchers/lichsugiao/templates/table.html',
		controller:['$scope','$modal','dmkh','dmvat','$timeout',function($scope,$modal,dmkh,dmvat,$timeout){
			...
			$scope.addDetail = function(){
				...
				$scope.dt_current ={line:line,lan_giao:1,ngay_giao:new Date()};
				if(!$scope.ngData){
					$scope.ngData =[];
				}
				$scope.openDetail('lg');
			...
			$scope.openDetail = function (size,template) {
				var modalInstance = $modal.open({
				  templateUrl: 'modules/vouchers/lichsugiao/templates/edit.html',
