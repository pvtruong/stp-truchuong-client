angular.module('lichsugiuModule',[])
	.directive('ngLichsugiu',[function(){
	return {
		restrict:'E',
		scope:{
			ngData:'=',
		...
		templateUrl:'templates/vouchers/lichsugiu/templates/table.html',
		controller:['$scope','$modal','dmkh','dmvat','$timeout',function($scope,$modal,dmkh,dmvat,$timeout){
			...
			$scope.addDetail = function(){
				...
				$scope.dt_current ={line:line,lan_giu:1,ngay_giu:new Date()};
				if(!$scope.ngData){
					$scope.ngData =[];
				}
				$scope.openDetail('lg');
			...
			$scope.openDetail = function (size,template) {
				var modalInstance = $modal.open({
				  templateUrl: 'templates/vouchers/lichsugiu/templates/edit.html',
